/* ═══════════════════════════════════════════════
   THE EPISTOLARY LIBRARY — PWA Onboarding
   Native install flow, offline awareness,
   first-run ceremony
   ═══════════════════════════════════════════════ */
...
(function() {
  ...
  style.textContent = `
    ...
    .pwa-welcome-icon {
      ...
      opacity: 0;
      transform: scale(0.8);
      animation: pwaWelcomeIn 0.8s 0.2s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }
    .pwa-welcome h2 {
      font-family: 'Cormorant Garamond', Palatino, serif;
      font-size: 1.6rem;
      ...
      opacity: 0;
      animation: pwaWelcomeIn 0.8s 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }
    .pwa-welcome p {
      font-family: 'Crimson Pro', Georgia, serif;
      font-size: 0.95rem;
      ...
      font-style: italic;
      opacity: 0;
      animation: pwaWelcomeIn 0.8s 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }
    .pwa-welcome-offline {
      margin-top: 1.5rem;
      font-family: 'JetBrains Mono', monospace;
      ...
      opacity: 0;
      animation: pwaWelcomeIn 0.8s 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }
    @keyframes pwaWelcomeIn {
  ...
  window.addEventListener('beforeinstallprompt', e => {
    e.preventDefault();
    deferredPrompt = e;
    // Show banner after user has spent a moment
  ...
  if (isStandalone) {
    ...
    // Don't show install banners in standalone mode
    return;
  }
  ...
  function scheduleInstallBanner() {
    ...
    if (store.dismissedAt && (Date.now() - store.dismissedAt) < 7 * 24 * 60 * 60 * 1000) return;
    // Don't show if already installed
    if (store.installed) return;

    // Wait for user engagement: 20s on page or significant scroll
    let shown = false;
    const timer = setTimeout(() => { if (!shown) { shown = true; showBanner(); } }, 20000);
  ...
  function showBanner() {
    ...
    if (isIOS && isSafari) {
      // iOS: button leads to instruction overlay
      bannerEl.innerHTML = `
      ...
      bannerEl.innerHTML = `
        <div class="pwa-banner-inner">
          <div class="pwa-banner-title">Read offline, like a real book</div>
          <div class="pwa-banner-text">
            Install the Library as an app. Your treatises travel with you — no internet required.
          </div>
          <div class="pwa-banner-actions">
      ...
      // Desktop without prompt: gentle nudge
      bannerEl.innerHTML = `
        <div class="pwa-banner-inner">
          <div class="pwa-banner-title">Take the Library with you</div>
          <div class="pwa-banner-text">
            This site works offline. Bookmark it, or look for "Install" in your browser menu to add it as an app.
          </div>
          <div class="pwa-banner-actions">
            <button class="pwa-btn-dismiss" id="pwa-dismiss-btn" style="flex:1">Understood</button>
          </div>
        </div>`;
    }
      ...
      <div class="pwa-ios-card">
        ...
        <div class="pwa-ios-steps">
          ...
          <div class="pwa-ios-step">
            ...
            <span>Tap <strong style="color:#e4c98a">"Add"</strong> — the Library appears as an app on your home screen</span>
          </div>
        </div>
  ...
  window.addEventListener('offline', () => showToast('Reading offline', 'offline'));

  // Show offline confirmation once on first load if SW is active
  ...
  // ── If no beforeinstallprompt fires (iOS, Firefox) ──
  // Schedule banner based on platform detection alone
